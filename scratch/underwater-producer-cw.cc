...
int main(int argc, char *argv[])
{
	...
	uint32_t n_Distance = 10; // not used right now
	...
	uint32_t n_Interval2 = 500;  // sending data interval
	cmd.AddValue("interval2", "interval2", n_Interval2);

	uint32_t n_Interval3 = 700;  // sending data interval
	...
	positionAlloc_Masters->Add(Vector(0.0, 0.0, n_Depth));
	...
	for (NodeContainer::Iterator i = nodes_Masters.Begin(); i != nodes_Masters.End(); i++)
	{
		...
		socket.SetPhysicalAddress(device->GetAddress());
	}
	...
	/*
	 Ptr<PacketSocketClient> client = CreateObjectWithAttributes<PacketSocketClient>(
	 "Interval", TimeValue(Seconds(n_Interval)),
	 ...
	 nodes_Sensors.Get(1)->AddApplication(client);
	 client->TraceConnectWithoutContext("Tx", MakeCallback(&SendPacket));
	 client2->TraceConnectWithoutContext("Tx", MakeCallback(&SendPacket));
	 nodes_Sensors.Get(2)->AddApplication(client2);
	 */
	...
	for (NodeContainer::Iterator i = nodes_Sensors.Begin(); i != nodes_Sensors.End(); i++)
	{
		...
		(*i)->AddApplication(client1);
		//(*i)->AddApplication(client2);
		//(*i)->AddApplication(client3);
	}
	...
	nodes_Masters.Get(0)->AddApplication(server);
