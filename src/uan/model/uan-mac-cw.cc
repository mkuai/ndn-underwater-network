...
UanMacCw::Enqueue(Ptr<Packet> packet, const Address &dest, uint16_t protocolNumber)
{
	switch (m_state)
	{
		...
		case RUNNING:
			...
			NS_LOG_DEBUG("Timer is running, put in queue");
			m_pktWaitingList.push_back(std::make_pair(packet, dest));
			//return false;
			break;
		...
		case TX:
			...
			if (m_phy->IsStateBusy())
			{
				...
				m_state = CCABUSY;
				uint32_t cw = (uint32_t) m_rv->GetValue(1, m_cw);
				m_savedDelayS = Seconds((double) (cw) * m_slotTime.GetSeconds());
				m_sendTime = Simulator::Now() + m_savedDelayS;
				NS_LOG_DEBUG(
						"Time " << Simulator::Now ().GetSeconds () << ": Addr " << GetAddress () << ": Enqueuing new packet while busy:  (Chose CW " << cw << ", Sending at " << m_sendTime.GetSeconds () << " Packet size: " << packet->GetSize ());
				NS_ASSERT(m_phy->GetTransducer()->GetArrivalList().size() >= 1 || m_phy->IsStateTx());
				StartTimer();
			}
			else
			{
				...
				NS_LOG_DEBUG(
						"Time " << Simulator::Now ().GetSeconds () << ": Addr " << GetAddress () << ": Enqueuing new packet while idle (sending), pid = " << packet->GetUid() << ", Sending at = " << m_sendTime.GetSeconds () << " Packet size: " << packet->GetSize ());
				NS_ASSERT(m_phy->GetTransducer()->GetArrivalList().size() == 0 && !m_phy->IsStateTx());
				//m_state = TX;
				m_state = RUNNING;
...
UanMacCw::NotifyTxStart(Time duration)
{
	...
	NS_LOG_DEBUG("Time " << Simulator::Now ().GetSeconds () << " scheduling TxEndEvent with delay "
			<< duration.GetSeconds () << ", state = " << m_state);
	if (m_state == RUNNING)
	{
		NS_ASSERT(0);
		m_state = CCABUSY;
...
UanMacCw::EndTx(void)
{
	...
	{
		m_state = IDLE;
		if (!m_pktWaitingList.empty())
		{
			std::pair<Ptr<Packet>, Address> pair = m_pktWaitingList.front();
			m_pktWaitingList.pop_front();
			Enqueue(pair.first, pair.second, 0);
		}
	}
...
{
	NS_LOG_FUNCTION(Simulator::Now().GetSeconds() << mode);
...
{
	NS_LOG_DEBUG(
			"Time " << Simulator::Now ().GetSeconds () << " Addr " << GetAddress () << " Saving timer (Delay = " << (m_savedDelayS = m_sendTime - Simulator::Now ()).GetSeconds () << ")");
	NS_ASSERT(m_pktTx);
...
UanMacCw::SendPacket(void)
{
	...
	if (m_state == IDLE)
	{
		NS_LOG_DEBUG("Time " << Simulator::Now ().GetSeconds () << " Addr " << GetAddress () << ": in IDLE, cancel sending");
		return;
	}

	if (m_state == CCABUSY)
	{
		NS_LOG_DEBUG("Time " << Simulator::Now ().GetSeconds () << " Addr " << GetAddress () << ": in CCABUSY, cancel sending");
		return;
	}

	if (m_state == TX)
	{
		NS_LOG_DEBUG("Time " << Simulator::Now ().GetSeconds () << " Addr " << GetAddress () << ": in TX, cancel sending");
		return;
	}
