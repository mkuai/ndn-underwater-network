...
UanPhyCalcSinrDefault::CalcSinrDb(Ptr<Packet> pkt,
	...
	//NS_LOG_DEBUG(
	//		"Calculating SINR:  RxPower = " << rxPowerDb << " dB.  Number of interferers = " << arrivalList.size() << "  Interference + noise power = " << totalIntDb
...
UanPhyCalcSinrFhFsk::CalcSinrDb(Ptr<Packet> pkt,
	...
	//NS_LOG_DEBUG(
	//		"Calculating SINR:  RxPower = " << rxPowerDb << " dB.  Effective Rx power " << effRxPowerDb << " dB.  Number of interferers = " << arrivalList.size()
	//		<< "  Interference + noise power = " << totalIntDb << " dB.  SINR = " << effRxPowerDb - totalIntDb << " dB.");
...
{
	NS_LOG_FUNCTION(Simulator::Now().GetSeconds() << m_mac->GetAddress());
	...
	NS_LOG_FUNCTION(Simulator::Now().GetSeconds() << m_mac->GetAddress()
			<< " notifying listeners in time " << Seconds(txdelay).GetSeconds());
...
UanPhyGen::StartRxPacket(Ptr<Packet> pkt, double rxPowerDb, UanTxMode txMode, UanPdp pdp)
{
	...
	{
		case TX:
			NS_LOG_DEBUG("PHY " << m_mac->GetAddress() << ":  packet comes when TX");
			NotifyRxDrop(pkt);    // traced source netanim
			NS_ASSERT(false);
			break;
		case RX:
		...
		case CCABUSY:
			...
			if (newsinr > m_rxThreshDb)
			{
				...
				m_pktRxPdp = pdp;
				double txdelay = pkt->GetSize() * 8.0 / txMode.GetDataRateBps();
				NS_LOG_DEBUG("PHY " << m_mac->GetAddress() << ": Delay = " << txdelay);
...
UanPhyGen::RxEndEvent(Ptr<Packet> pkt, double rxPowerDb, UanTxMode txMode)
{
	...
	if (pkt != m_pktRx)
	{
		NS_LOG_DEBUG(this << ": Not received pkt");
		return;
	}
	...
	if (GetInterferenceDb((Ptr<Packet> ) 0) > m_ccaThreshDb)
	{
		NS_LOG_FUNCTION(Simulator::Now().GetSeconds() <<"BUSY ");
		m_state = CCABUSY;
		NotifyListenersCcaStart();
	}
	else
	{
		NS_LOG_FUNCTION(Simulator::Now().GetSeconds() <<"IDLE ");
		m_state = IDLE;
		UpdatePowerConsumption(IDLE);
	}
	...
	{
		NS_LOG_FUNCTION(Simulator::Now().GetSeconds() <<"OK ");
	...
	{
		NS_LOG_FUNCTION(Simulator::Now().GetSeconds() <<"ERROR ");
